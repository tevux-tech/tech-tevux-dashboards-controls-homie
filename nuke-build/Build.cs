...
class Build : NukeBuild {
    ...
    static string ProjectBaseName => "Tech.Tevux.Dashboards.Controls.Homie";
    static string RepositoryRootName => "tech-tevux-dashboards-controls-homie";
    static AbsolutePath OutputDirectory => RootDirectory / "nuke-build" / "output";
    static AbsolutePath NuspecFilename => RootDirectory / ProjectBaseName / $"{ProjectBaseName}.nuspec";
    static AbsolutePath ProjectFilename => RootDirectory / ProjectBaseName / $"{ProjectBaseName}.csproj";
    ...
    Target UpdateVersions => _ => _
        ...
        .Executes(() => {
            ...
            File.WriteAllText(ProjectFilename, assemblyInfoContent);
        });
    ...
    Target Pack => _ => _
        ...
        .Executes(() => {
            ...
            settings = settings.SetProperty("repository", $"https://github.com/tevux-tech/{RepositoryRootName}");
            settings = settings.SetProperty("projectUrl", $"https://github.com/tevux-tech/{RepositoryRootName}");
